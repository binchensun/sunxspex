...
Replica from sunxspex/emission_orig.py, but try to simplify the integration
...
def bremsstrahlung_thin_target(eph, edist='bkpl', params=None, efd = True):
    ...
    flux: np.array
        ...
        The coefficient a0 is calculated as a0 = nth * V * nnth, where
            nth: plasma density (unit: cm^-3)
            V: volume of source (unit: cm^3)
            nnth: Integrated nonthermal electron flux density (unit: cm^-2 s^-1), if efd = True, or
                  Integrated electron number density (unit: cm^-3), if efd = False
