...
def test_brem_thicktarget():
    ...
    assert np.allclose(res, [3.5282883164459861975673291444470203760637592388101219288332650651371110170457542922313942321266289070536004146561026573181152343750000000000000000000000000000000e-34,
                             ...
                             2.6828146475529079795700327856387287763742212421289479098429606686084675195201084428187451054412101727228938787342404914681992522673681378364562988281250000000000e-43],
                            rtol=1e-8, atol=0.)

def test_brem_thintarget():
    photon_energies = np.array([5, 10, 50, 150, 300, 500, 750, 1000], dtype=np.float64)
    (p, q, eelow, eebrk, eehigh) = (5, 5, 10, 1000, 10000)
    res = emission.bremsstrahlung_thin_target(photon_energies, edist='bkpl', params=(p, q, eelow, eebrk, eehigh))
    # IDL code to generate values taken from cross flux
    ...
    assert np.allclose(res, res_idl, rtol=1e-8, atol=0.)

def test_brem_thintarget_discrete():
    photon_energies = 10 ** np.linspace(np.log10(10.), np.log10(1000.),100)
    (p, q, eelow, eebrk, eehigh) = (5, 5, 10, 1000, 10000)
    # SSWIDL-like calculation using broken power-law function form and gaussian quadrature integration
    start = time.time()
    res = emission.bremsstrahlung_thin_target(photon_energies, edist='bkpl', params=(p, q, eelow, eebrk, eehigh))
    end = time.time()
    print('SSWIDL-like calculation takes {0:.5f} s'.format(end - start))
    electron_energies = 10 ** np.linspace(np.log10(10.), np.log10(10000.), 1000)
    # Calculation using sampled broken power-law and scipy.integrate.trapz()
    start = time.time()
    electron_dist = emission.broken_powerlaw(electron_energies, p, q, eelow, eebrk, eehigh)
    res_discrete = emission.bremsstrahlung_thin_target(photon_energies, edist='discrete',
                                               params=(electron_energies, electron_dist))
    end = time.time()
    err = np.abs(res-res_discrete)/np.abs(res)
    print('Discrete calculation takes {0:.5f} s'.format(end - start))
    print('Max and Median Relative errors: {0:.2e}, {1:.2e}'.format(np.nanmax(err), np.median(err)))
    assert np.allclose(res, res_discrete, rtol=0.02, atol=0.)
